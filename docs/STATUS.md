...
| **Documents / E-sign** | `/documents` (+ `[id]`, `/new`, `/upload`); public `/sign/[token]` | ✅ Central document store + DocuSign-like signing. Affiliate contracts + customer SOWs, consent + audit trail (IP/UA/UTC/SHA-256), executed PDF with a Certificate of Completion, expiring single-use tokens. **Uploaded PDFs** (`/documents/upload`, migration 0039): send an externally-authored PDF (a contract already signed by our side) for a third party to counter-sign — `pdf-lib` stamps the typed signature onto the page (in the document's own signature block via `signature_spec`, or an appended signature page) + appends the Certificate; the signer reads the real PDF (iframe, signed URL), not an HTML re-render; content hash anchors to the original bytes. `kind='other'` + `source_path`. |
...
## Data / schema
Postgres on Supabase (`@supabase/ssr`, cookie sessions). **Migrations `0001`–`0039`** applied (plain SQL in
