...
| **Operations · Inventory** | `/operations/inventory` (+ `[id]`, `/locations`) | ✅ Bulk + serialized units + maintenance; structured locations (warehouse rows / offsite); per-item & per-unit photos; CSV import. **List now shows live "in use now / available now"** (reservations covering today, netted against on-hand) + **reserve to an event straight from the list** (per-row modal → same `event_items` write). **Bulk "assign to location"** (multi-select → set storage location on items + their units). **Mistake-safe item delete** (hard delete only when the item has no reservation history; otherwise retire to keep history). |
